...
class MyAdapter(private val data: ArrayList<Contact>) :
    ...
    class ViewHolder(v: View): RecyclerView.ViewHolder(v) {
        ...
        val tv_te = v.findViewById<TextView>(R.id.tv_te)
        val img_delete = v.findViewById<ImageView>(R.id.img_delete)
        val pho = itemView.findViewById<ImageView>(R.id.photo)
        fun bindView(photo:Photo){
            pho.setImageResource(photo.imageSrc)
        }
    }
    //回傳資料數量
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.tv_te.text = data[position].te
        //設定監聽器，使用 removeAt()刪除指定位置的資料
        holder.img_delete.setOnClickListener {
            data.removeAt(position)
            notifyDataSetChanged()
        }
        holder.bindView(data[position].photo2)
    }
}